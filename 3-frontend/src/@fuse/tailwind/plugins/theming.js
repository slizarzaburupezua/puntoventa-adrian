...
const theming = plugin.withOptions((options) => ({
    ...
    {
        /**
         * Create userInfoLogueado themes object by going through the provided themes and
         * merging them with the provided "default" so, we can have a complete
         * set of color palettes for each userInfoLogueado theme.
         */
        const userInfoLogueadoThemes = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            themeName,
         ...
         */
        let themes = _.fromPairs(_.map(userInfoLogueadoThemes, (theme, themeName) => [
            themeName,
                ...
                _.fromPairs(_.map(theme, (palette, paletteName) => [
                    paletteName,
                    {
                        ...
                        contrast: _.fromPairs(_.map(generateContrasts(palette), (color, hue) => [
                            hue,
                            _.get(userInfoLogueadoThemes[themeName], [`on-${paletteName}`, hue]) || color,
                        ])),
                    },
                ])),
                ['primary', 'accent', 'warn'],
        ...
        const sassMap = jsonToSassMap(JSON.stringify({'userInfoLogueado-themes': themes}));

        /* Get the file path */
        const filename = path.resolve(__dirname, ('../../styles/userInfoLogueado-themes.scss'));
        ...
        /**
         * Iterate through the userInfoLogueado's themes and build Tailwind components containing
