...
{
    public class MarcasInfrastructureService : IMarcasInfrastructureService
    {
        public async Task<byte[]> GenerarReportePorMarcasAsync(FiltroReportePorMarcas filtro)
        {
            string filePath = Path.Combine(Directory.GetCurrentDirectory(), "templates", "reportes", "ventas", "porMarcas.xlsx");
            ...
            {
                var worksheet = workbook.Worksheet("Marcas");
                ...
                // Llenar ventas de la marca en A27 y montos en B27
                int rowInitial = 27;
                int rowNext = 30;
                foreach (var detalleVentaMarca in filtro.LstDetalleTotalMarcas)
                {
                    ...
                    worksheet.Cell(rowInitial, 1).Value = detalleVentaMarca.NombreMarca; // Columna A
                    ...
                    worksheet.Cell(rowInitial, 2).Value = detalleVentaMarca.TotalVenta; // Columna B
                ...
                // Ordenar las marcas por TotalVentas de mayor a menor
                var marcasOrdenadas = filtro.LstDetalleTotalMarcas
                    .OrderBy(c => c.TotalVenta)
                    .ToList();

                // Llenar ventas de la marcas en A27 y montos en B27
                rowInitial = 27;
                foreach (var marca in marcasOrdenadas)
                {
                    ...
                    worksheet.Cell(rowInitial, 4).Value = marca.NombreMarca; // Columna D
                    ...
                    worksheet.Cell(rowInitial, 5).Value = marca.TotalVenta; // Columna E
                ...
                // Agrupar las marcas por Fecha y sumar los totales
                var ventasAgrupadosPorFecha = filtro.LstDetalleVenta
                    .GroupBy(marca => marca.FECHA_REGISTRO.Date) // Agrupa por fecha
                    .Select(grupoMarca => new
                    {
                        Fecha = grupoMarca.Key,
                        TotalVenta = grupoMarca.Sum(x => x.PRECIO_TOTAL)
                    })
