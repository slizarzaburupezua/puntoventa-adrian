...
namespace PERFISOFT.VENTASPLATFORM.INFRASTRUCTURE.DTO.Inventario.Producto.Validator
{
    public class RegistrarProductoValidator : AbstractValidator<RegistrarProductoRequest>
    {
        ...
        {
            Log.Information("Iniciando validación Request RegistrarProductoRequest");
            ...
            RuleFor(x => x.Codigo)
                .NotNull().WithMessage(GV.PropertyNameNotNull)
                .Length(Numeracion.Dos, Numeracion.Veinte).WithMessage(GV.PropertyNameCorrectLenght)
                .NotEmpty().WithMessage(GV.PropertyNameNotEmpty);
        ...
        public override ValidationResult Validate(ValidationContext<RegistrarProductoRequest> context)
        {
